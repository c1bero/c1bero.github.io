...
The above structure has a size of 40 bytes. Let's see how and when such a structure is created in the kernel.
...
`fcntl(pipe_fd, F_SETPIPE_SZ, PAGE_SIZE * n)` results in allocating a buffer of size `n * sizeof(struct pipe_buffer) = n * 40` for the above `bufs`
circular array of pipe buffers. For example, `fcntl(pipe_fd, F_SETPIPE_SZ, 4096 * 2)` leads to the allocation of a buffer of size `2 * 40 = 80` for
the `bufs` member.
...
`GFP_KERNEL_ACCOUNT | __GFP_NOWARN`. In our example `n=2`, a buffer of size `2 * 40 = 80` is allocated. If `CONFIG_MEMCG_KMEM` was enabled, this
...
I hope that this writeup will be helpful for some readers when trying to understand and apply this pipe buffer technique.
